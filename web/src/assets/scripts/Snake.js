...
export class Snake extends AcGameObject {
    constructor(info, gamemap) {
        ...
        this.speed = 5;  //定义速度，表示每秒移动5个格子
        this.direction = -1; //-1表示没有指令，0,1,2,3表示上右下左
        this.status = "idle"; //idle表示静止，move表示移动中，die表示死亡
    ...
    next_step() { //将蛇的状态变为下一步
        ...
        //蛇移动的方式，蛇头抛出去向指令方向移动，蛇尾向前面的部分方向移动，其余部分不动
        const k = this.cells.length;
        //数组向后移动一位，这样就多出来了一个蛇头，然后去移动
        for (let i = k; i > 0; i--) {
            this.cells[i] = JSON.parse(JSON.stringify(this.cells[i - 1]));
        }

        if (!this.gamemap.check_valid(this.next_cell)) {  //蛇直接去世
            this.status = "die";
        }
    }
    ...
    render() {
        ...
        for (let i = 1; i < this.cells.length; i++) {
            ...
            if (Math.abs(a.x - b.x) < this.eps && Math.abs(a.y - b.y) < this.eps)
                continue;
            if (Math.abs(a.x - b.x) < this.eps) {  //纵方向
                //a.x - 0.5 + 0.1   加上10%偏移量
